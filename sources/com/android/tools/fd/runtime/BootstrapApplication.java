...
public class BootstrapApplication extends Application {
    ...
    public BootstrapApplication() {
        if (android.util.Log.isLoggable(LOG_TAG, 2)) {
            android.util.Log.v(LOG_TAG, String.format("BootstrapApplication created. Android package is %s, real application class is %s.", new Object[]{AppInfo.applicationId, AppInfo.applicationClass}));
        }
    }
    ...
    private static void setupClassLoaders(Context context, String codeCacheDir, long apkModified) {
        ...
        Class<Server> cls = Server.class;
        Class<MonkeyPatcher> cls2 = MonkeyPatcher.class;
        if (!dexList.isEmpty()) {
            if (android.util.Log.isLoggable(LOG_TAG, 2)) {
                android.util.Log.v(LOG_TAG, "Bootstrapping class loader with dex list " + join(10, dexList));
            }
            ...
            try {
                ...
                if (processes == null || processes.size() <= 1) {
                    ...
                    while (true) {
                        ...
                        RunningAppProcessInfo processInfo = (RunningAppProcessInfo) i$.next();
